...
int main (int argc, char** argv)
{
  ...
  for (Long64_t iEvent = 0 ; true ; ++iEvent) 
	...
	{ 
      ...
      std::vector< tuple<float, int, int> > VBFcand_dEta;
	  ...
	  if (theBigTree.jets_px->size ()>1){
	    for (unsigned int iJet = 0 ;   (iJet < theBigTree.jets_px->size ()) && (theSmallTree.m_njets < maxNjetsSaved) ;  ++iJet){
	      ...
	      for (unsigned int kJet = iJet+1 ;   (kJet < theBigTree.jets_px->size ()) && (theSmallTree.m_njets < maxNjetsSaved) ;  ++kJet){
                ...
                VBFcand_dEta.push_back(make_tuple(fabs(ijet.Eta()-kjet.Eta()),iJet,kJet));
            }
	    }
        
        if (VBFcand_dEta.size()>0)
        {
            std::sort(VBFcand_dEta.begin(), VBFcand_dEta.end());
            VBFidx1n = std::get<1>(*(VBFcand_dEta.rbegin()));
            VBFidx2n = std::get<2>(*(VBFcand_dEta.rbegin()));
            TLorentzVector VBFjet1n;
            VBFjet1n.SetPxPyPzE(
				 theBigTree.jets_px->at (VBFidx1n),
				 theBigTree.jets_py->at (VBFidx1n),
				 theBigTree.jets_pz->at (VBFidx1n),
				 theBigTree.jets_e->at  (VBFidx1n)
			);
	        TLorentzVector VBFjet2n;
            VBFjet2n.SetPxPyPzE(
				 theBigTree.jets_px->at (VBFidx2n),
				 theBigTree.jets_py->at (VBFidx2n),
				 theBigTree.jets_pz->at (VBFidx2n),
				 theBigTree.jets_e->at  (VBFidx2n)
			);
            bool   hasgj1_VBFn = false;
            bool   hasgj2_VBFn = false;
            
            if (isMC)
            {
                int mcind = theBigTree.jets_genjetIndex->at(VBFidx1n);
                if (mcind>=0)
                  {
                    ...
                    {
                      hasgj1_VBFn = true;
                      theSmallTree.m_VBFgenjet1n_pt =  thisGenJet.Pt() ;
                      theSmallTree.m_VBFgenjet1n_eta = thisGenJet.Eta();
                      theSmallTree.m_VBFgenjet1n_phi = thisGenJet.Phi();
                      theSmallTree.m_VBFgenjet1n_e =   thisGenJet.E();
                    }
                  }
                mcind = theBigTree.jets_genjetIndex->at(VBFidx2n);
                if (mcind>=0)
                {
                  TLorentzVector thisGenJet(theBigTree.genjet_px->at(mcind),theBigTree.genjet_py->at(mcind),theBigTree.genjet_pz->at(mcind),theBigTree.genjet_e->at(mcind));
                  if (thisGenJet.Pt() > 8)
                  {
                     hasgj2_VBFn = true;
                     theSmallTree.m_VBFgenjet2n_pt =  thisGenJet.Pt() ;
                     theSmallTree.m_VBFgenjet2n_eta = thisGenJet.Eta();
                     theSmallTree.m_VBFgenjet2n_phi = thisGenJet.Phi();
                     theSmallTree.m_VBFgenjet2n_e =   thisGenJet.E();
                  }
                }
	        }
            theSmallTree.m_VBFjj_mass_n = (VBFjet1n+VBFjet2n).M();
            theSmallTree.m_VBFjj_mass_n_log = log((VBFjet1n+VBFjet2n).M());
            theSmallTree.m_VBFjj_deltaEta_n = std::get<0>(*(VBFcand_dEta.rbegin()));

            theSmallTree.m_VBFjet1n_pt = VBFjet1n.Pt() ;
            theSmallTree.m_VBFjet1n_eta = VBFjet1n.Eta();
            theSmallTree.m_VBFjet1n_phi = VBFjet1n.Phi();
            theSmallTree.m_VBFjet1n_e = VBFjet1n.E();
            theSmallTree.m_VBFjet1n_btag =  (theBigTree.bCSVscore->at (VBFidx1n)) ;
            theSmallTree.m_VBFjet1n_flav =  (theBigTree.jets_HadronFlavour->at (VBFidx1n)) ;
            theSmallTree.m_VBFjet1n_hasgenjet= hasgj1_VBFn ;

            theSmallTree.m_VBFjet2n_pt= VBFjet2n.Pt() ;
            theSmallTree.m_VBFjet2n_eta= VBFjet2n.Eta();
            theSmallTree.m_VBFjet2n_phi= VBFjet2n.Phi();
            theSmallTree.m_VBFjet2n_e= VBFjet2n.E();
            theSmallTree.m_VBFjet2n_btag=  (theBigTree.bCSVscore->at (VBFidx2n)) ;
            theSmallTree.m_VBFjet2n_flav=  (theBigTree.jets_HadronFlavour->at (VBFidx2n)) ;
            theSmallTree.m_VBFjet2n_hasgenjet= hasgj2_VBFn ;

            theSmallTree.m_VBFjj_HT_n = VBFjet1n.Pt()+VBFjet2n.Pt();

            theSmallTree.m_dau1_z_n = getZ(tlv_firstLepton.Eta(),VBFjet1n.Eta(),VBFjet2n.Eta());
            theSmallTree.m_dau2_z_n = getZ(tlv_secondLepton.Eta(),VBFjet1n.Eta(),VBFjet2n.Eta());

            theSmallTree.m_bjet1_z_n = getZ(tlv_firstBjet.Eta(),VBFjet1n.Eta(),VBFjet2n.Eta());
            theSmallTree.m_bjet2_z_n = getZ(tlv_secondBjet.Eta(),VBFjet1n.Eta(),VBFjet2n.Eta());

            theSmallTree.m_tauH_z_n = getZ(tlv_tauH.Eta(),VBFjet1n.Eta(),VBFjet2n.Eta());
            theSmallTree.m_bH_z_n = getZ(tlv_bH.Eta(),VBFjet1n.Eta(),VBFjet2n.Eta());

            theSmallTree.m_HH_z_n = getZ(tlv_HH.Eta(),VBFjet1n.Eta(),VBFjet2n.Eta());
        }
